...
USWCMyDataTableFactory::USWCMyDataTableFactory()
{
	bCreateNew = true;
	bEditAfterNew = true;
	bEditorImport = true;
...
FText USWCMyDataTableFactory::GetDisplayName() const
{
	return LOCTEXT("MyDataTableFactory", "My Data Table");
}

UObject* USWCMyDataTableFactory::FactoryCreateNew(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
	check(DoesSupportClass(InClass));
	return NewObject<UObject>(InParent, InClass, InName, Flags);
}

bool USWCMyDataTableFactory::DoesSupportClass(UClass* Class)
{
