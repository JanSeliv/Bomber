...
void UGfpmAction_AddComponents::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	ComponentList[AddedIndex].AdditionFlags = DefaultAdditionFlags;
}

// Adds new entry to component list
void UGfpmAction_AddComponents::AddComponentEntry(TSubclassOf<AActor> ActorClass, TSubclassOf<UActorComponent> ComponentClass, bool bClientComponent, bool bServerComponent)
{
	if (!ensureMsgf(ActorClass, TEXT("ASSERT: [%i] %hs:\n'ActorClass' is null!"), __LINE__, __FUNCTION__)
	    || !ensureMsgf(ComponentClass, TEXT("ASSERT: [%i] %hs:\n'ComponentClass' is null!"), __LINE__, __FUNCTION__))
	{
		return;
	}

	FGameFeatureComponentEntry& NewEntryRef = ComponentList.AddDefaulted_GetRef();
	NewEntryRef.ActorClass = ActorClass;
	NewEntryRef.ComponentClass = ComponentClass;
	NewEntryRef.bClientComponent = bClientComponent;
	NewEntryRef.bServerComponent = bServerComponent;
	NewEntryRef.AdditionFlags = DefaultAdditionFlags;
